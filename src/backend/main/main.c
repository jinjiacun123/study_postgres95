 ...
 *
 * IDENTIFICATION
 *    $Header: /usr/local/cvsroot/postgres95/src/backend/main/main.c,v 1.1.1.1 1996/07/09 06:21:32 scrappy Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <string.h>
...
main(int argc, char *argv[])
{
    ...
    len = strlen(argv[0]);
    if(len >= 10 
	&& ! strcmp(argv[0] + len - 10, "postmaster"))
	...
	&& strcmp(argv[1], "-boot") == 0) {
      ...
      //exit(PostgresMain(argc, argv));
		...
		char query_string[4096] = "select * from test";	
		Oid *typev;
		//InitPostgres(DBName);
		pg_eval(query_string, argv, typev, argc);
		exit(1);
	}
}
