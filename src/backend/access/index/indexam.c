	     ...
	     IndexTuple indexTuple)
{
    ...
    procedure = relation->rd_am->aminsert;
    if(! RegProcedureIsValid(procedure))
       elog(WARN, "index_%s: invalid %s regproc", CppAsString(insert), CppAsString(aminsert));
...
index_delete(Relation relation, ItemPointer indexItem)
{
    ...
    procedure = relation->rd_am->amdelete;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: invalid %s regproc", CppAsString(delete), CppAsString(amdelete));
		...
		ScanKey key)
{
    ...
    procedure = relation->rd_am->ambeginscan;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: invalid %s regproc", CppAsString(beginscan), CppAsString(ambeginscan));
...
index_rescan(IndexScanDesc scan, bool scanFromEnd, ScanKey key)
{
    ...
    procedure = scan->relation->rd_am->amrescan;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: invalid %s regproc ", CppAsString(rescan), CppAsString(amrescan));
...
index_endscan(IndexScanDesc scan)
{
    ...
    procedure = scan->relation->rd_am->amendscan;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: isvalid %s regproc", CppAsString(endscan), CppAsString(amendscan));
...
index_markpos(IndexScanDesc scan)
{
    ...
    procedure = scan->relation->rd_am->ammarkpos;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: isvalid %s regproc", CppAsString(markpos), CppAsString(ammarkpos));
...
index_restrpos(IndexScanDesc scan)
{
    ...
    procedure = scan->relation->rd_am->amrestrpos;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: isvalid %s regproc", CppAsString(restrpos), CppAsString(amrestrpos));
	      ...
	      ScanDirection direction)
{
    ...
    procedure = scan->relation->rd_am->amgettuple;
    if(! RegProcedureIsValid(procedure))
      elog(WARN, "index_%s: invalid %s regproc", CppAsString(getnext), CppAsString(amgettuple));
